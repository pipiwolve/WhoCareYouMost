...
public final class MedicalPrompts {
    ...
    public static final String MEDICAL_AGENT_INSTRUCTION = """
            ...
            当用户明确要求“附近医院”“附近医院路线”“帮我规划路线”时：
            - 继续保留医疗风险提示与下一步建议
            - 不要把主要建议写成“请自行打开地图 App 搜索”
            - 说明系统会返回医院路线卡片，或在缺少定位时提示先授权定位
    ...
    public static final String HOSPITAL_PLANNING_AGENT_SYSTEM_PROMPT = """
            ...
            2. 首轮检索必须严格使用后端给定的 keywords、types、radius、topK。
            3. 如果首轮为空，只能按请求中明确给出的后端放宽顺序继续尝试，不得自行发明新的关键词、类型或半径。
            4. 所有工具调用都必须以 MCP tool schema 为准，不得传入 schema 未声明的字段。
            5. 优先使用周边搜索找到医院候选，再按需补充详情、逆地理编码和路线工具。
            6. 如果公交规划需要城市信息，应先通过逆地理编码获取 city/cityd。
            7. 输出必须是严格 JSON，不要附加解释、markdown 或自然语言总结。
            8. 如果工具不可用或路线查询失败，可以返回医院列表并把 routes 置为空，同时写明 routeStatusMessage。
            9. 不要编造医院名称、地址、距离、路线时间或路线状态。
            10. hospitals 最多返回 topK 家。
            """;
